...
**How it Works**
1. **Get a swap quote**: Check the expected output before committing — no gas. `pancakeswap-v2-plugin quote --token-in USDT --token-out CAKE --amount-in <amount>`
2. **Execute the swap**: Send input token and receive output — ERC-20 approval fires automatically if needed. `pancakeswap-v2-plugin swap --token-in USDT --token-out CAKE --amount-in <amount> --confirm`
...
5. **Add liquidity**: Deposit both tokens to receive LP tokens representing your share of the pool. `pancakeswap-v2-plugin add-liquidity --token-a CAKE --token-b BNB --amount-a <amount-a> --amount-b <amount-b> --confirm`
