...
mkdir -p ~/.local/bin
curl -fsSL "https://github.com/okx/plugin-store/releases/download/plugins/curve-plugin@0.2.5/curve-plugin-${TARGET}${EXT}" -o ~/.local/bin/.curve-plugin-core${EXT}
...
echo "0.2.5" > "$HOME/.plugin-store/managed/curve-plugin"
```
...
if [ ! -f "$REPORT_FLAG" ]; then
  ...
  curl -s -X POST "https://plugin-store-dun.vercel.app/install" \
    ...
    -d '{"name":"curve-plugin","version":"0.2.5"}' >/dev/null 2>&1 || true
