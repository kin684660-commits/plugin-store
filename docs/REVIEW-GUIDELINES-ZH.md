...
本文档描述了向 OKX Plugin Store 提交Plugin必须通过的审核流程。了解这些标准有助于您提交规范的Plugin并避免延误。
...
此外，`skill-review.yml` 工作流在每次 PR 和推送到 `main` 时独立运行，对官方Plugin执行静态检查和轻量级 AI 审核。
...
| 检查项 | 行为 |
|--------|------|
| **每个 PR 只能修改一个Plugin** | PR 只能修改一个 `skills/<name>/` 目录中的文件。修改多个Plugin将导致检查失败。 |
| **不得修改 `skills/` 之外的文件** | PR 不得修改 `skills/` 目录树之外的任何文件。 |
| **名称唯一性** | 对于新Plugin，名称不能与 `registry.json` 中已有的名称重复。 |
...
| 类别 | 检查项 | 严重程度 |
|------|--------|----------|
| **结构** | Plugin根目录下存在 `plugin.yaml` | Error |
| **结构** | 存在 `SKILL.md`（针对 skill 类型Plugin） | Error |
| **结构** | `plugin.yaml` 包含有效的 YAML | Error |
| **版本一致性** | `plugin.yaml` 中的版本与 `SKILL.md` frontmatter 中声明的版本一致 | Error |
| **安全默认值** | 设置了 `PAUSED=True`（Plugin不得自动启动） | Error |
...
| **名称唯一性** | 新Plugin名称不与现有注册表条目冲突 | Error |
...
| 条件 | 添加标签 | 移除标签 |
|------|----------|----------|
| 新Plugin提交 | `new-plugin` | -- |
| 更新现有Plugin | `plugin-update` | -- |
...
合并报告将作为 PR 评论发布，显示：
- Plugin名称、语言和源仓库及 commit SHA
- 构建结果（通过/失败/跳过）
- 源 commit 链接，便于审计

### 跳过构建的情况

如果 `plugin.yaml` 中没有 `build` 部分（例如纯 SKILL.md Plugin，无编译二进制文件），Phase 2 将被完全跳过。
...
审核前，工作流会克隆最新的 onchainos 源代码（`okx/onchainos-skills` main 分支）作为审核上下文。这确保 AI 准确知道哪些 CLI 命令存在，并能验证Plugin是否正确引用了真实的 onchainos 功能。

### 九项审计维度

| # | 维度 | 评估内容 |
|---|------|----------|
| 1 | **Plugin概览** | 名称、版本、分类、作者、许可证、风险级别，以及Plugin功能的通俗描述。 |
...
| 6 | **源代码审查** | 语言和构建配置、依赖审计、代码安全检查（硬编码密钥、未声明的网络请求、文件系统访问、动态代码执行、unsafe 块）。仅适用于包含源代码的Plugin。 |
...
| **L-MALI：恶意意图** | Critical | Plugin声明用途与实际行为之间的差异 -- 例如"钱包追踪器"暗中上传私钥。 |
| **L-MEMA：记忆投毒** | High | 尝试写入 Agent 记忆文件（MEMORY.md、SOUL.md）以植入跨会话后门。 |
| **L-IINJ：外部请求通知** | Info/Medium | 外部 API 或 CLI 调用。如果Plugin声明了不可信数据边界则为 Info；未声明则为 Medium。 |
...
| **TF004** | 不可验证依赖 + 检测到恶意意图 | High | 恶意Plugin安装额外未验证包，其 postinstall 钩子执行攻击载荷。 |
...
2. **生成 SUMMARY.md** -- 由 Claude 生成的Plugin通俗描述，用于注册表展示。
...
| 文件 | 用途 |
|------|------|
| `SUMMARY.md` | 面向用户的Plugin通俗描述，用于商店展示 |
...
人工审核在通过 Phase 3 后的 **1 至 3 个工作日**内完成。复杂或高风险Plugin如需额外审核者可能需要更长时间。
...
| 6 | **持久化机制（cron、launchctl、systemd）** | 后台服务在Plugin卸载后仍然存活，可作为长期后门。 |
| 7 | **访问敏感文件（~/.ssh、~/.aws、~/.kube、~/.gnupg）** | 没有Plugin有正当理由读取 SSH 密钥或云凭证。 |
