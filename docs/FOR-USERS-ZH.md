...
欢迎！本指南面向所有希望在 AI 助手中使用Plugin的用户。
无需编程经验。只要你能给 AI 聊天机器人发消息，就能使用Plugin。
...
3. [快速上手](#3-快速上手)
4. [Plugin目录](#4-Plugin目录)
5. [使用示例](#5-使用示例)
6. [管理你的Plugin](#6-管理你的Plugin)
7. [安全与免责声明](#7-安全与免责声明)
8. [常见问题](#8-常见问题)
...
安装新应用后会获得新功能一样，你的 AI 助手安装Plugin后也会获得新能力。

- **Plugin Store** = 应用商店
- **一个Plugin** = 一个应用
- **安装Plugin** = 在手机上下载应用

没有Plugin时，你的 AI 助手可以回答问题、处理一般性任务。有了Plugin，它可以在
去中心化交易所兑换代币、追踪资深交易员的操作、帮你规划流动性头寸等等。

### Plugin的结构

商店中的每个Plugin都包含三个核心文件：

- **plugin.yaml** -- Plugin的元数据（名称、版本、作者、描述）
- **SKILL.md** -- 教 AI 助手如何使用该Plugin的指令文件
- **.claude-plugin/plugin.json** -- Plugin系统的配置文件

你无需关心这些文件的细节。这里提及只是为了让你了解安装Plugin时背后发生了什么。

### 预检自动注入

通过 Plugin Store 安装Plugin时，系统会在后台自动配置你所需的一切：

- **onchainos CLI** -- 链上操作系统的命令行工具
- **onchainos Skills** -- 区块链交互的核心 AI 技能
- **plugin-store Skill** -- Plugin管理技能本身
- **安装报告** -- 已安装和已配置内容的摘要

这些无需手动安装，在每次会话中首次安装Plugin时会自动注入。

### 实际效果

下面展示安装和使用Plugin的过程。你只需用自然语言描述需求，AI 助手会完成剩余工作：

```
你:     我想在 Uniswap 上规划一笔代币兑换。

AI:     我可以帮你！让我先安装 Uniswap Swap Planner Plugin。
...
### 每个Plugin都经过 4 阶段安全审核

在任何Plugin上架之前，都必须通过四层检查：

1. **自动化代码扫描** -- 程序会扫描Plugin中是否存在已知的危险模式（如试图窃取
   密码或注入隐藏命令的代码）。可以把它想象成机场安检扫描仪。

2. **AI 驱动的行为分析** -- AI 审核员会阅读整个Plugin，确保它确实在做它声称要做的
   事，而不是别的。这能发现简单扫描器可能遗漏的问题。

3. **毒性流检测** -- 系统会检查多个小动作组合起来是否会形成有害行为。例如，
   一个既读取敏感文件又向互联网发送数据的Plugin会被标记，即使每个单独的动作
   看起来都无害。

4. **人工审核** -- 真人在Plugin上线前进行审核。自动化工具很好，但人的判断力能
   发现机器无法捕捉的问题。

### 三个信任等级告诉你Plugin的来源

不是所有Plugin都来自同一个地方。我们为每个Plugin标注来源，让你了解其出处：
...
不过，Official 和 Verified Partner 级别的Plugin来自有良好声誉的团队。

### 三个风险等级告诉你Plugin能做什么

每个Plugin都被分配了一个风险等级，让你知道会发生什么：
...
安装Plugin只需一条命令。无需下载文件，无需编辑配置文件，无需重启任何东西。
一行命令，即可开始使用。
...
将 `uniswap-swap-planner` 替换为你想要的任何Plugin名称。（参见下方的
[Plugin目录](#4-Plugin目录)查看全部 16 个Plugin的完整列表。）
...
你的 AI 助手会识别你的需求，激活合适的Plugin，并引导你完成操作。如果涉及交易，
...
Plugin Store 目前包含 **16 个Plugin**。以下是帮你找到合适Plugin的方法。

### 按需求查找

| 我想... | Plugin | 安装命令 | 风险等级 |
|---------|------|---------|---------|
| 浏览和安装其他Plugin | Plugin Store | `npx skills add okx/plugin-store --skill plugin-store` | Starter |
...
| 运行 Rust CLI 端到端测试 | Test Rust CLI | `npx skills add okx/plugin-store --skill rust-cli-inspector` | Starter |

### 按风险等级查找

每个Plugin都有一个风险等级，告诉你它能做什么以及你需要注意什么：

#### Starter（入门级）-- 安全探索

这些Plugin只**读取**信息。它们帮助你规划、学习和浏览，绝不触碰你的钱包或发起交易。

*示例：浏览可用Plugin、规划兑换方案（不执行）、阅读安全文档。*
...
这些Plugin可以**准备交易**，但在任何操作发生前始终会请求你的确认。可以把它想象成
购物车：Plugin把商品加入购物车，但由你决定是否结账。
...
这些Plugin运行**自动化交易策略**。它们可以根据信号、排名或市场模式代你买卖代币。
这就像把你的交易设为自动驾驶。

*示例：自动买入聪明钱钱包正在买入的代币、狙击排行榜上新上线的代币、扫描
meme 代币机会。*

**预期体验：** 这些Plugin可以用真金白银执行真实交易。它们提供模拟交易模式
...
以下是三个由简入深的真实场景，展示Plugin在实际中如何工作。
...
AI:     欢迎！以下是 Plugin Store 中目前可用的内容：
        ...
        Starter 级别的Plugin（Swap Planner、Liquidity Planner）是很好的起点。
        它们只展示信息——不会买卖任何东西。
...
**发生了什么：** 你浏览了Plugin、安装了一个，并获取了兑换信息。全程没有任何
资金风险。这就是 Starter 级别Plugin的体验。
...
AI:     我可以使用 Smart Money Signal Copy Trade Plugin帮你设置。
...
所有Plugin管理都通过 `npx skills add` 命令或用自然语言询问 AI 助手来完成。
...
或者直接说：*"帮我安装 Uniswap Swap Planner Plugin。"*
...
或者说：*"重新安装 Uniswap AI Plugin以获取最新版本。"*

### 获取特定Plugin的详情

询问你的 AI 助手：*"给我介绍一下 Uniswap AI Plugin。"*

### 按关键词搜索Plugin

询问你的 AI 助手：*"有没有用于兑换代币的Plugin？"*

### 列出所有可用Plugin

询问你的 AI 助手：*"显示所有可用的Plugin。"*
...
OKX Plugin Store 中的每个Plugin在发布前都会经过 **4 阶段安全审核**：

| 阶段 | 做了什么 | 能发现什么 |
|------|---------|-----------|
| 1. 自动化代码扫描 | 程序检查危险的代码模式 | 恶意软件、密码窃取、隐藏命令 |
| 2. AI 行为分析 | AI 阅读整个Plugin以验证其功能符合声明 | 欺骗性Plugin、隐藏功能 |
| 3. 毒性流检测 | 系统检查看似无害的动作组合是否会产生有害结果 | 隐藏在表面之下的复杂攻击 |
| 4. 人工审核 | 真人审查提交的内容 | 自动化工具遗漏的任何问题 |

所有Plugin——Official、Verified Partner 和 Community——都经过相同的流程。

### 你需要了解的

虽然我们努力确保 Plugin Store 的安全，但有一些重要的事情需要理解：

- **这不是投资建议。** Plugin Store 及其Plugin是工具。它们不会告诉你该买什么、
  卖什么或投资什么。任何交易决定都由你自己做出。

- **自己做好研究（DYOR）。** 在使用任何涉及真钱的Plugin之前，花时间了解它的
  功能、风险以及是否适合你的情况。

- **加密货币波动剧烈。** 代币价格可以在很短的时间内大幅上涨或下跌。你可能
  损失部分或全部资金。

- **从模拟模式开始。** Advanced 级别的交易Plugin提供使用模拟资金的练习模式。
  务必先试用这个。
...
**是的。** Plugin Store 和其中所有Plugin均可免费安装和使用。部分Plugin与区块链网络
交互时你可能需要支付网络手续费（称为"Gas 费"），但Plugin本身不收取任何费用。

### 安全吗？

每个Plugin在上架前都经过 **4 阶段安全审核**（自动化扫描、AI 分析、毒性流检测和
人工审核）。我们还为每个Plugin标注信任徽章（Official、Verified Partner 或
Community），让你了解其来源。

也就是说，没有任何审核流程是完美的。对于涉及真钱的Plugin，请务必从模拟模式开始，
并自己做好研究。

### Plugin Store 收集什么数据？

**不收集任何数据。** Plugin Store 本身不收集任何个人数据、使用数据或钱包信息。
它只是一个将Plugin文件下载到你电脑上的工具——仅此而已。

个别Plugin可能会与区块链网络交互（区块链本身是公开的），但 Plugin Store 本身
不会追踪你。

### Plugin安装在哪里？

Plugin安装在你电脑上 AI 助手的本地配置文件夹中。它们只是文本文件（给 AI 的指令），
存储在你的 AI 助手的其他设置旁边。
...
**不需要。** 你通过自然语言与 AI 助手交流来使用Plugin。AI 会处理所有技术细节。
你只需描述你想做什么。
...
**可以。** 你可以安装任意数量的Plugin。你的 AI 助手会根据你的需求自动使用合适的
Plugin。例如，如果你问关于兑换代币的问题，它会使用兑换Plugin。如果你问关于预测市场
的问题，它会使用 Polymarket Plugin。
...
2. **查看已知问题：** 访问Plugin的 GitHub 页面查看已知问题
...
交易策略。Plugin会模拟可能发生的情况——追踪买入、卖出、盈利和亏损——但不会发生
实际交易。这是在投入真实资金前评估 Advanced 级别Plugin的最安全方式。

### 什么是"Gas 费"？

Gas 费是你向区块链网络支付的小额费用，用于处理你的交易。可以把它想象成寄信的
邮费——你支付少量费用来让你的交易被传送和记录。Gas 费会根据网络活跃度而变化。
只读取信息的Plugin（Starter 级别）不会产生 Gas 费。

### 有多少个Plugin可用？

目前 Plugin Store 中有 **16 个Plugin**，涵盖 DeFi 交易、流动性管理、预测市场
和自动化交易策略等多个类别。
